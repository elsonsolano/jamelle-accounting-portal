...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="w-8 h-8 bg-emerald-100 rounded-lg flex items-center justify-center text-emerald-600 text-sm">₱</span>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        ...
        <p class="text-2xl font-bold text-red-600 tabular-nums">
            ₱{{ number_format($totalExpensesThisMonth, 0) }}
        </p>
        <p class="text-xs text-gray-400 mt-1">{{ $now->format('F Y') }}</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div class="flex items-center justify-between mb-3">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Branches Active</p>
            <span class="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center text-blue-600 text-sm font-bold">#</span>
        </div>
        <p class="text-2xl font-bold text-blue-700">
            {{ $periodsReporting }} <span class="text-base font-normal text-gray-400">/ {{ $branches->count() }}</span>
        </p>
        <p class="text-xs text-gray-400 mt-1">periods open this month</p>
    </div>

</div>

{{-- ── Main Content: Branch Table + Recent Activity ────────────────────────── --}}
<div class="flex gap-6 items-start">

    {{-- Left: Branch performance this month --}}
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('expense-periods.index') }}"
                   class="text-xs text-indigo-600 hover:underline">View all periods →</a>
            </div>
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($branches as $branch)
                        @php
                            $period = $currentPeriods->firstWhere('branch_id', $branch->id);
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-8 text-center text-gray-400 text-sm">No branches found.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if($branches->count() > 0)
                <tfoot class="bg-gray-50 border-t-2 border-gray-200 text-sm font-semibold">
                    <tr>
                        ...
                        <td class="px-5 py-3 text-right tabular-nums text-red-600">
                            ₱{{ number_format($totalExpensesThisMonth, 2) }}
                        </td>
                        <td class="px-5 py-3 text-right tabular-nums
                                   {{ $operatingIncomeThisMonth >= 0 ? 'text-indigo-700' : 'text-red-600' }}">
        ...
        <div class="mt-4 grid grid-cols-2 gap-4">
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm px-5 py-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold text-emerald-700 tabular-nums mt-0.5">
                        ₱{{ number_format($allTimeSales, 2) }}
                    </p>
                </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm px-5 py-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold text-red-600 tabular-nums mt-0.5">
                        ₱{{ number_format($allTimeExpenses, 2) }}
                    </p>
                </div>
                <span class="text-2xl opacity-20">📊</span>
            </div>
    ...
    <div class="w-80 shrink-0 space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-4 py-3.5 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('expense-periods.index') }}"
                   class="text-xs text-emerald-600 hover:underline">View all →</a>
            </div>
